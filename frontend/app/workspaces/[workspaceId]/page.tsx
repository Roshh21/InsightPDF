...
export default function WorkspaceOverviewPage() {
  ...
  const loadAll = useCallback(async () => {
    try {
      const [ws, docs] = await Promise.all([api.getWorkspace(workspaceId), api.listDocuments(workspaceId)]);
      setWorkspace(ws);
      setDocuments(docs);
    } catch (e) {
      setError(e instanceof ApiError ? e.message : "Failed to load workspace.");
    }
  }, [workspaceId]);
  ...
  async function handleUpload(files: File[]) {
    ...
    try {
      ...
      if (result.errors && result.errors.length > 0) setUploadErrors(result.errors);
      await loadAll();
    } catch (e) {
  ...
  async function handleDeleteWorkspace() {
    if (!confirm(`Delete workspace "${workspace?.name}" and all its documents? This cannot be undone.`)) return;
  ...
  return (
    <div className="mx-auto max-w-6xl px-8 py-10">
      <PageHeader
        eyebrow={`${workspace.stats.total_documents} document${workspace.stats.total_documents === 1 ? "" : "s"}`}
        title={workspace.name}
        description={workspace.description || undefined}
        actions={
          <button className="btn-secondary text-danger hover:bg-danger-soft" onClick={handleDeleteWorkspace}>
      ...
      {statusCounts && Object.keys(statusCounts).length > 0 && (
        <div className="mb-6 flex flex-wrap gap-2">
          {Object.entries(statusCounts).map(([status, count]) => (
            <div key={status} className="rounded-[5px] border border-line bg-paper-soft px-2.5 py-1 font-mono text-[11px] text-ink-soft">
      ...
      {uploadErrors.length > 0 && (
        <div className="mb-6 rounded-md border border-danger/30 bg-danger-soft p-3 animate-fade-in">
          <div className="mb-1.5 flex items-center justify-between">
            ...
            <button onClick={() => setUploadErrors([])} className="text-danger hover:opacity-70">
              <X size={14} />
            </button>
          </div>
      ...
      {documents.length === 0 ? (
        <EmptyState icon={FileText} title="No documents yet" description="Drop one or more PDFs above to get started." />
      ) : (
